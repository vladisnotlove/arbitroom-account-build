...
// helpers 
var createTileElement = function (props) {
    var tileElement = document.createElement("canvas");
    tileElement.style.position = "absolute";
    tileElement.style.top = "0px";
    tileElement.style.left = "0px";
    tileElement.style.willChange = "transform";
    tileElement.style.width = "".concat(props.size.x, "px");
    tileElement.style.height = "".concat(props.size.y, "px");
    tileElement.width = props.size.x;
    tileElement.height = props.size.y;
    return tileElement;
};
var areRectsIntersected = function (r1, r2) {
    return !(r2.topLeft.x > r1.bottomRight.x ||
        r2.bottomRight.x < r1.topLeft.x ||
        r2.topLeft.y > r1.bottomRight.y ||
        r2.bottomRight.y < r1.topLeft.y);
};
var isRectIntersected = function (rect, point) {
    return point.x > rect.topLeft.x && point.x < rect.bottomRight.x &&
        point.y > rect.topLeft.y && point.y < rect.bottomRight.y;
};
var cached = [
    hexMargin + hexSize.x * 0.25,
    hexMargin + hexSize.x * 0.75,
    hexMargin + hexSize.x
];
var isHexIntersected = function (hexPosition, point) {
    var polygon = [
    ...
    return (0, point_in_polygon_1.default)([point.x, point.y], polygon);
};
var drawHexagon = function (ctx, position, options) {
    ...
    ctx.closePath();
    ctx.fillStyle = options.background || "#060119";
    ctx.fill();
    if (options.text) {
        ctx.font = "12px Golos";
        ctx.fillStyle = "#fff";
...
var drawPartnerHexagon = function (ctx, position, partner) {
    var centerX = position.x + hexMargin + hexSize.x * 0.5;
    ...
    if (partner.backgroundSrc) {
        var img_1 = new Image;
        img_1.src = partner.backgroundSrc;
        img_1.onload = function () {
            var imgSize = hexSize.add(hexMargin * 2);
            ctx.beginPath();
            ctx.moveTo(position.x + cached[0], position.y + hexMargin);
            ctx.lineTo(position.x + cached[1], position.y + hexMargin);
            ctx.lineTo(position.x + cached[2], position.y + hexMargin + hexSize.y * 0.5);
            ...
            ctx.closePath();
            ctx.clip();
            ctx.drawImage(img_1, position.x, position.y, imgSize.x, imgSize.y);
            drawText();
            img_1.remove();
        };
    }
...
var drawHexagonShadow = function (ctx, position, partner) {
    if (partner.theme) {
        ctx.beginPath();
        ctx.moveTo(position.x + cached[0], position.y + hexMargin);
        ctx.lineTo(position.x + cached[1], position.y + hexMargin);
        ...
        ctx.shadowBlur = 48;
        ctx.fillStyle = "#060119";
        ctx.fill();
    }
};
var getHexPositionsAround = function (position) {
    var p1 = new Vector_1.default(position.x + hexPeriod.x * 0.5, position.y + hexPeriod.y * 0.5);
    var p2 = new Vector_1.default(position.x, position.y + hexPeriod.y);
    var p3 = new Vector_1.default(position.x - hexPeriod.x * 0.5, position.y + hexPeriod.y * 0.5);
    var p4 = new Vector_1.default(position.x - hexPeriod.x * 0.5, position.y - hexPeriod.y * 0.5);
    var p5 = new Vector_1.default(position.x, position.y - hexPeriod.y);
    var p6 = new Vector_1.default(position.x + hexPeriod.x * 0.5, position.y - hexPeriod.y * 0.5);
    return [p1, p2, p3, p4, p5, p6];
};
var getHexPositionsBySpiral = function (start, length) {
    var toHash = function (position) { return "".concat(position.x, " ").concat(position.y); };
    var positions = new Queue_1.default();
    var hashes = new Set();
    var result = [];
    ...
    for (var i = 0; i < length; i++) {
        ...
        if (position) {
            ...
            getHexPositionsAround(position).forEach(function (p) {
                var hash = toHash(p);
                if (!hashes.has(hash)) {
                    hashes.add(hash);
                    positions.enqueue(p);
...
// main
var addStructurePartners = function () {
    try {
        // get elements
        var structurePartners = document.querySelector(".structure-partners");
        var hexs_1 = document.querySelector(".structure-partners__hexs");
        var hexsInner_1 = document.querySelector(".structure-partners__hexs-inner");
        if (!(hexs_1 instanceof HTMLElement))
            throw Error(".structure-partners has no .structure-partners__hexs");
        if (!(hexsInner_1 instanceof HTMLElement))
            throw Error(".structure-partners has no .structure-partners__hexs-inner");
        hexsInner_1.innerHTML = "";
        hexsInner_1.style.transformOrigin = "0 0";
        // get props
        var str = (structurePartners === null || structurePartners === void 0 ? void 0 : structurePartners.getAttribute("data-dataset")) || "";
        var partners_1 = str ? JSON.parse(str) || [] : [];
        // set state
        var scale_1 = 0.8;
        var size_1 = new Vector_1.default(hexs_1.clientWidth, hexs_1.clientHeight).divide(scale_1);
        var tileSize_1 = size_1.multiply(1.5);
        var position_1 = tileSize_1.multiply(0.5).subtract(size_1.multiply(0.5));
        var tiles_1 = [
            {
                position: new Vector_1.default(0, 0),
            }
        ];
        var start = position_1.add(size_1.multiply(0.5)).divide(hexPeriod).floor().multiply(hexPeriod);
        var partnerPositions_1 = getHexPositionsBySpiral(start, partners_1.length);
        // helpers
        var resetTiles_1 = (0, lodash_1.debounce)(function () {
        ...
        var zoom_1 = function (zoomPosition, newScale) {
            ...
            if (newScale > maxScale)
                return;
            var deltaPosition = position_1.subtract(zoomPosition);
            var scaledDeltaPosition = deltaPosition.divide(scale_1).multiply(newScale);
            position_1 = position_1.add(scaledDeltaPosition.subtract(deltaPosition));
            scale_1 = newScale;
            size_1 = new Vector_1.default(hexs_1.clientWidth, hexs_1.clientHeight).divide(newScale);
        ...
        hexs_1.addEventListener("wheel", function (event) {
            if (event.ctrlKey) {
                ...
                var mouseViewPosition = new Vector_1.default(event.pageX, event.pageY);
                var zoomPosition = position_1.subtract(mouseViewPosition.subtract(hexsViewPosition).divide(scale_1));
                var newScale = scale_1 + scale_1 * scaleStep * Math.sign(event.deltaY) * -1;
                zoom_1(zoomPosition, newScale);
            }
        });
        var isMouseMoving_1 = false;
        var startMouseEvent_1;
        var prevMouseEvent_1;
        hexs_1.addEventListener("mousedown", function (event) {
            event.preventDefault();
            startMouseEvent_1 = event;
        ...
        window.addEventListener("mousemove", function (event) {
            ...
            var movement = new Vector_1.default(event.pageX - prevMouseEvent_1.pageX, event.pageY - prevMouseEvent_1.pageY).divide(scale_1).multiply(-1);
        ...
        hexs_1.addEventListener("click", function (event) {
            if (!startMouseEvent_1)
                return;
            var deltaPageX = Math.abs(startMouseEvent_1.pageX - event.pageX);
            var deltaPageY = Math.abs(startMouseEvent_1.pageY - event.pageY);
            var deltaMove = Math.sqrt(deltaPageX * deltaPageX + deltaPageY * deltaPageY);
            if (deltaMove > 4) {
                ...
                var hexsViewPosition = Vector_1.default.from(hexs_1.getBoundingClientRect());
                var mouseViewPosition = new Vector_1.default(event.pageX, event.pageY);
                var clickPosition_1 = position_1.add(mouseViewPosition.subtract(hexsViewPosition).divide(scale_1));
                partners_1.forEach(function (partner, index) {
                    ...
                    if (isRectIntersected(hexRect, clickPosition_1)) {
                        if (isHexIntersected(partnerPosition, clickPosition_1)) {
                            var message = JSON.stringify({
                                source: routes_1.default.structurePartners.path(),
                                type: "open-details",
                                payload: {
                                    partner: partner
                                }
        ...
        hexsInner_1.addEventListener("touchmove", function (event) {
            ...
            if (event.touches.length === 2 && prevTouchDiff_1) {
                ...
                var touch1 = new Vector_1.default(event.touches[0].pageX, event.touches[0].pageY);
                var touch2 = new Vector_1.default(event.touches[1].pageX, event.touches[1].pageY);
                var touchDiff = touch1.subtract(touch2).length;
                var delta = touchDiff - prevTouchDiff_1;
            ...
            if (touches.length === 1 && touchState_1 !== "zoom") {
                ...
                if (delta.length < 4) {
                    ...
                    partners_1.forEach(function (partner, index) {
                        ...
                        if (isRectIntersected(hexRect, touchPosition_1)) {
                            if (isHexIntersected(partnerPosition, touchPosition_1)) {
                                var message = JSON.stringify({
                                    source: routes_1.default.structurePartners.path(),
                                    type: "open-details",
                                    payload: {
...
exports["default"] = addStructurePartners;
...
var Queue = /** @class */ (function () {
    ...
    Queue.prototype.dequeue = function () {
        if (this.isEmpty())
            return null;
        var itemToBeRemoved = this.first;
        if (this.first === this.last) {
            this.last = null;
        }
        this.first = this.first.next;
        this.size--;
        return (itemToBeRemoved === null || itemToBeRemoved === void 0 ? void 0 : itemToBeRemoved.value) || null;
    };
