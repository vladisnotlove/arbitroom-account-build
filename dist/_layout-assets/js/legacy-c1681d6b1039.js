...
var addAllNotifications = function () {
    document.querySelectorAll(".all-notifications__body").forEach(function (allNotifications) {
        allNotifications.addEventListener("click", function (e) {
            if (!(e.target instanceof HTMLElement))
                return;
            var notification = e.target.closest(".all-notifications__notification");
            if (notification) {
...
exports["default"] = addAllNotifications;
...
var addCashoutPool = function () {
    ...
    $(expressToggler).find(".toggle-button").click(function (e) {
        var tooltipOn = $(expressToggler).find(".express-toggler__on-tooltip");
        var tooltipOff = $(expressToggler).find(".express-toggler__off-tooltip");
        if (e.currentTarget.getAttribute("data-value") === "express") {
            $(tooltipOn).addClass("active");
            $(tooltipOff).removeClass("active");
        }
        else {
            $(tooltipOff).addClass("active");
            $(tooltipOn).removeClass("active");
        }
    });
    $("#cashoutPoolBtn").click(function () {
        if ($(expressToggler).find(".toggle-button.active")[0].getAttribute("data-value") === "express") {
            $("#cashoutPoolExpressAlert").addClass("show");
            $("#cashoutPoolAlert").removeClass("show");
        }
        else {
            $("#cashoutPoolAlert").addClass("show");
            $("#cashoutPoolExpressAlert").removeClass("show");
        }
    });
};
exports["default"] = addCashoutPool;
...
var addChat = function () {
    ...
    $(".chat__close-btn").click(function () {
        $(".chat").toggleClass("open");
    });
};
...
var addCollapseTable = function () {
    $(".collapse-table").each(function (i, collapseTable) {
        ...
        var expandBtn = $(collapseTable).find(".collapse-table__expand-btn button").get(0);
        if (!collapseBtn || !expandBtn)
            return;
        $(expandBtn).click(function () {
            $(collapseTable).addClass("expand");
        });
        $(collapseBtn).click(function () {
            $(collapseTable).removeClass("expand");
        });
    });
};
...
var addLineList = function () {
    $(".line-list").each(function (i, lineList) {
        ...
        var trigger = $(lineList).find(".line-list__trigger");
        $(trigger).click(function () {
            $(lineList).toggleClass("open");
        });
        $(menu).click(function () {
            $(lineList).removeClass("open");
        });
        $(document.documentElement).click(function (e) {
            if ($(lineList).has(e.target).length === 0) {
                $(lineList).removeClass("open");
            }
        });
    });
};
exports["default"] = addLineList;
...
var addPrograms = function () {
    $(".programs").each(function (i, programs) {
        ...
        var collapseBtn = $(programs).find(".programs__collapse-btn button").get(0);
        if (!expandBtn || !collapseBtn)
            return;
        $(expandBtn).click(function () {
            $(programs).addClass("expand");
        });
        $(collapseBtn).click(function () {
            $(programs).removeClass("expand");
        });
    });
};
exports["default"] = addPrograms;
...
var addPromoCard = function () {
    $(".promo-card").each(function (i, promoCard) {
        ...
        var expandBtn = $(promoCard).find(".promo-card__expand-btn").get(0);
        var collapseBtn = $(promoCard).find(".promo-card__collapse-btn").get(0);
        if (!more || !expandBtn || !collapseBtn)
            return;
        $(expandBtn).click(function () {
            $(more).addClass("expand");
            $(expandBtn).removeClass("active");
            $(collapseBtn).addClass("active");
            var animationTime = Math.sqrt(more.scrollHeight) / 20;
            $(more).css("height", "".concat(more.scrollHeight, "px"));
            $(more).css("transition", "height ".concat(animationTime, "s"));
        });
        $(collapseBtn).click(function () {
...
var addTabs = function () {
    $(".tabs").each(function (i, tabs) {
        var menu = tabs.querySelector(".tabs__menu");
        var menuItems = Array.from(tabs.querySelectorAll(".tabs__menu-item"));
        var moreItems = tabs.querySelector(".tabs__menu-more-items");
        var moreItemsMenu = tabs.querySelector(".tabs__menu-more-items-menu");
        var moreItemsMenuItems = Array.from(tabs.querySelectorAll(".menu__item"));
        var contentItems = Array.from(tabs.querySelectorAll(".tabs__content-item"));
        if (!menu || !moreItems || !(moreItemsMenu instanceof HTMLElement))
            return;
        var setActive = function (index) {
            var _a, _b, _c;
            menuItems.forEach(function (item) { return item.classList.remove("active"); });
            moreItemsMenuItems.forEach(function (item) { return item.classList.remove("selected"); });
            contentItems.forEach(function (item) { return item.classList.remove("active"); });
            (_a = menuItems[index]) === null || _a === void 0 ? void 0 : _a.classList.add("active");
            (_b = moreItemsMenuItems[index]) === null || _b === void 0 ? void 0 : _b.classList.add("selected");
            (_c = contentItems[index]) === null || _c === void 0 ? void 0 : _c.classList.add("active");
        ...
        var updateMenu = (0, throttle_debounce_1.throttle)(250, function () {
            var visibleWidth = moreItems.clientWidth;
            var lastVisible = -1;
            ...
            moreItemsMenuItems.forEach(function (item, index) {
                if (index > lastVisible) {
        ...
        var openMoreItemsMenu = function () {
            overlay = $("<div class='tabs__overlay'></div>")
                ...
                $(moreItemsMenu).removeClass("open");
            })
                .prependTo("body");
            $(moreItemsMenu).addClass("open");
            popper.update();
        };
        var closeMoreItemsMenu = function () {
            $(overlay).remove();
            $(moreItemsMenu).removeClass("open");
        };
        var toggleMoreItemsMenu = function () {
            if ($(moreItemsMenu).hasClass("open")) {
                closeMoreItemsMenu();
            }
            else {
                openMoreItemsMenu();
        ...
        // set initial state
        updateMenu();
        var activeIndex = menuItems.findIndex(function (menuItem) { return menuItem.classList.contains("active"); }) || 0;
        setActive(activeIndex);
        // add handlers
        new ResizeObserver(function () { return updateMenu(); }).observe(menu);
        moreItems.addEventListener("click", function () {
            toggleMoreItemsMenu();
        });
        moreItemsMenuItems.forEach(function (currentMenuItem, index) {
            currentMenuItem.addEventListener("click", function () {
                closeMoreItemsMenu();
                setActive(index);
            });
        });
        menuItems.forEach(function (currentMenuItem, index) {
            currentMenuItem.addEventListener("click", function () {
                setActive(index);
            });
        });
...
var addWithLinkedSelect = function () {
    document.querySelectorAll(".with-linked-select").forEach(function (elem) {
        if (!(elem instanceof HTMLElement))
            return;
        var str = elem.getAttribute("data-linked-select");
        var linkedSelect = str ? document.querySelector(str) : undefined;
        if (linkedSelect) {
            linkedSelect.addEventListener("click", function (e) {
                if (!(e.target instanceof HTMLElement))
                    return;
                var value = e.target.getAttribute("data-value");
                if (value === undefined)
                    return;
                var elemItem = elem.querySelector("[data-value=".concat(value, "]"));
                if (!(elemItem instanceof HTMLElement))
                    return;
                elemItem.click();
            });
            elem.addEventListener("click", function (e) {
                ...
                if (value === undefined)
                    return;
                var selectItem = linkedSelect.querySelector("[data-value=".concat(value, "]"));
                if (!(selectItem instanceof HTMLElement))
                    return;
                selectItem.click();
            });
        }
...
exports["default"] = addWithLinkedSelect;
